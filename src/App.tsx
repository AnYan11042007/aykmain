...
import { get, ref, onValue, set, onDisconnect, push, update } from 'firebase/database';
...
import LiveCasinoStream from './components/LiveCasinoStream';
...
import DailyMissionsPortal from './components/DailyMissionsPortal';
import S88NewsPortal from './components/S88NewsPortal';
...
import WorldCardBattleModal from './components/modals/WorldCardBattleModal';
import AIBotSimulation from './components/AIBotSimulation';
...
export default function App() {
  ...
  return (
    <div className="flex min-h-screen bg-transparent text-slate-200 antialiased selection:bg-[#00ff80]/20 selection:text-[#00ff80] relative">
      ...
      <main className="flex-1 min-h-screen pl-0 md:pl-72 transition-all duration-300">
        <div className={`max-w-7xl mx-auto px-4 md:px-8 py-6 md:py-8 transition-all duration-300 ${
          ...
          {currentTab === 'hunting' && (
            <WorldHuntingPortal uid={uid} user={user} onShowResult={handleShowGameResult} />
          )}
          ...
          {currentTab === 'missions' && (
            <DailyMissionsPortal uid={uid} user={user} onShowResult={handleShowGameResult} />
          )}
          ...
          {currentTab === 'tx_history' && (
            <TransactionHistoryPortal uid={uid} user={user} />
          )}
      ...
      {/* World Card Battle 1v1 & Spectate */}
      {activeGameKey === 'world_card_battle' && (
        <WorldCardBattleModal 
          uid={uid} 
          user={user} 
          onClose={() => setActiveGameKey('')} 
          onShowResult={handleShowGameResult}
      ...
      {/* Anti-cheat lockdown modal */}
      {isCheaterLocked && (
        <div className="fixed inset-0 bg-black/95 flex flex-col items-center justify-center z-[100000] p-6 text-center select-none font-mono">
          <div className="max-w-md bg-gradient-to-b from-red-950/40 to-black border-2 border-red-500 rounded-2xl p-8 shadow-[0_0_50px_rgba(239,68,68,0.4)]">
            <span className="text-6xl block mb-4 animate-bounce">🚨</span>
            <h1 className="text-red-500 font-black text-xl tracking-wider uppercase mb-4 text-glow-red animate-pulse">
              HỆ THỐNG PHÁT HIỆN GIAN LẬN (F12)
            </h1>
            <p className="text-[#8b949e] text-xs leading-relaxed mb-6">
              Tài khoản của bạn đã có hành vi chỉnh sửa trái phép số PP bằng console hoặc bộ nhớ Client.
            </p>
            <div className="text-left bg-red-950/20 border border-red-500/30 rounded-xl p-4 space-y-2 text-[10px] text-red-300 mb-6">
              <p>• <b>Sự kiện:</b> Tự ý điều chỉnh số dư PP</p>
              <p>• <b>Trạng thái:</b> Tự động thu hồi & khôi phục số dư gốc</p>
              <p>• <b>Báo cáo:</b> Đã gửi thông tin định danh và log hành vi đến trang Admin</p>
            </div>
            <button
              onClick={() => window.location.reload()}
              className="w-full py-3 bg-red-600 hover:bg-red-500 text-white font-black text-xs rounded-xl uppercase tracking-widest transition shadow-[0_0_15px_rgba(239,68,68,0.3)] cursor-pointer"
            >
              [ TẢI LẠI TRANG ]
            </button>
          </div>
