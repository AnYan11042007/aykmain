...
const SHOP_ITEMS: ShopItem[] = [
  // Luxury Avatar Frames (200M to 1 Billion PP)
  { 
    id: 'frame_luxury_200m', 
    name: 'Khung Băng Tuyết Hoàng Kim (200M)', 
    type: 'frame', 
    price: 200000000, 
    value: 'https://png.pngtree.com/png-vector/20250802/ourmid/pngtree-game-ui-border-metal-frame-avatar-frame-gorgeous-ice-gold-frame-png-image_16991272.webp', 
    desc: 'Khung Băng Tuyết Hoàng Kim sang trọng đính đá pha lê tuyệt mỹ.' 
  },
  { 
    id: 'frame_luxury_500m', 
    name: 'Khung Hoàng Gia Ngọc Bích (500M)', 
    type: 'frame', 
    price: 500000000, 
    value: 'https://png.pngtree.com/png-clipart/20220313/original/pngtree-game-avatar-frame-metal-border-png-image_7434122.png', 
    desc: 'Khung Hoàng Gia viền kim loại nạm đá ngọc bích quý giá.' 
  },
  { 
    id: 'frame_luxury_750m', 
    name: 'Khung Rồng Thần Thượng Cổ (750M)', 
    type: 'frame', 
    price: 750000000, 
    value: 'https://png.pngtree.com/png-clipart/20240319/original/pngtree-avatar-frame-dragon-round-animal-template-for-game-cartoon-empty-dragon-png-image_14623493.png', 
    desc: 'Khung Rồng Vàng uy nghiêm biểu tượng cho quyền lực tuyệt đối.' 
  },
  { 
    id: 'frame_luxury_1b', 
    name: 'Khung Rồng Vương Cổ Đại (1 TỶ)', 
    type: 'frame', 
    price: 1000000000, 
    value: 'https://png.pngtree.com/png-clipart/20240319/original/pngtree-avatar-frame-dragon-round-animal-template-for-game-cartoon-empty-dragon-png-image_14623495.png', 
    desc: 'Khung Rồng Vương Thượng Cổ II giá trị 1 TỶ PP xịn xịn nhất hệ thống S88.' 
  },

  // Classic Frames
  { id: 'frame_gold', name: 'Khung Hoàng Gia Gold', type: 'frame', price: 15000, value: 'gold-ring', desc: 'Khung Avatar mạ vàng ròng 24K óng ánh sang trọng.' },
...
export default function MarketplacePortal({ uid, user, onShowResult }: MarketplacePortalProps) {
  ...
  const [customFrames, setCustomFrames] = useState<ShopItem[]>([]);

  const [giftTarget, setGiftTarget] = useState('');
  const [giftAmount, setGiftAmount] = useState('');
  const [giftMsg, setGiftMsg] = useState('');

  // Fetch custom avatar frames created by Admin
  useEffect(() => {
    const framesRef = ref(db, 'settings/custom_avatar_frames');
    const unsub = onValue(framesRef, (snap) => {
      if (snap.exists()) {
        const val = snap.val();
        setCustomFrames(Object.values(val) as ShopItem[]);
      } else {
        setCustomFrames([]);
      }
    });
  ...
  const allShopItems = [...SHOP_ITEMS, ...customFrames];
            ...
            return (
              ...
              >
                <div>
                  ...
                  {/* Frame Visual Live Preview */}
                  {item.type === 'frame' && (
                    <div className="mt-3 py-3 px-2 bg-black/40 rounded-xl flex items-center justify-center">
                      <div className="relative w-14 h-14 rounded-full flex items-center justify-center">
                        ...
                        {item.value.startsWith('http') ? (
                          <img 
                            src={item.value} 
                            alt="" 
                            className="absolute -inset-2 w-[calc(100%+16px)] h-[calc(100%+16px)] object-contain pointer-events-none filter drop-shadow-[0_0_10px_rgba(255,215,0,0.8)]" 
                            referrerPolicy="no-referrer"
                          />
                        ) : (
                          <div className={`absolute inset-0 rounded-full border-2 ${
                            item.value === 'gold-ring' ? 'border-[#ffd700] shadow-[0_0_12px_rgba(255,215,0,0.6)]' :
                            item.value === 'neon-ring' ? 'border-[#ff003c] shadow-[0_0_12px_rgba(255,0,60,0.6)] animate-pulse' :
                            'border-[#00f0ff] shadow-[0_0_12px_rgba(0,240,255,0.6)]'
                          }`} />
                        )}
                      </div>
                    </div>
            ...
            {myInventory.frames.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {myInventory.frames.map((frame) => {
                  ...
                  const shopMatch = allShopItems.find((s) => s.value === frame);
                  const frameName = shopMatch ? shopMatch.name : (frame === 'gold-ring' ? 'Hoàng Gia Gold' : frame === 'neon-ring' ? 'Cầu Vồng Neon' : 'Hologram Cyberpunk');
                        ...
                        <div className="relative w-10 h-10 rounded-full flex items-center justify-center shrink-0">
                          ...
                          {frame.startsWith('http') ? (
                            <img 
                              src={frame} 
                              alt="" 
                              className="absolute -inset-1.5 w-[calc(100%+12px)] h-[calc(100%+12px)] object-contain pointer-events-none filter drop-shadow-[0_0_8px_rgba(255,215,0,0.8)]" 
                              referrerPolicy="no-referrer"
                            />
                          ) : (
                            <div className={`absolute inset-0 rounded-full border-2 ${
                              frame === 'gold-ring' ? 'border-[#ffd700]' : frame === 'neon-ring' ? 'border-[#ff003c]' : 'border-[#00f0ff]'
                            }`} />
                          )}
                        </div>
                        <div>
                          <strong className="block text-white text-xs uppercase">{frameName}</strong>
                          <span className="text-[10px] text-slate-400">Khung trang bị làm nổi bật Avatar</span>
                        </div>
                      </div>
