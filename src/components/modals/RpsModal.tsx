...
export default function RpsModal({ uid, user, roomId, onClose, onShowResult }: RpsModalProps) {
  ...
  return (
    <div className="overlay z-[4500]">
      <div className="glass-box login-panel overflow-y-auto max-h-[90vh] w-[95vw] md:w-full max-w-[850px] p-6 border-[#00ff80] relative">
        <button 
        ...
        <div className="relative w-full aspect-[16/10] sm:aspect-[16/9] md:h-[400px] max-h-[75vh] min-h-[300px] bg-gradient-to-b from-[#1a1a2e] to-[#0d0d1a] border-4 border-[#00ff80] rounded-3xl overflow-hidden shadow-[inset_0_0_40px_rgba(0,0,0,0.9)] select-none mb-4">
          
          {/* Stake pot */}
          <div className="absolute top-[6%] sm:top-[12%] left-1/2 -translate-x-1/2 bg-black/60 py-1 sm:py-1.5 px-4 sm:px-6 border border-[#00ff80] text-[#00ff80] rounded-full text-[9px] sm:text-xs font-bold font-mono tracking-widest uppercase z-10">
            TỔNG TIỀN CƯỢC: {(room.bet * 2).toLocaleString()} PP
          </div>

          {/* Duel area */}
          <div className="absolute top-[22%] sm:top-[28%] left-0 w-full flex justify-around items-center px-4 sm:px-6">
            
            {/* Player 1 Card */}
            <div className="flex flex-col items-center gap-1.5 sm:gap-2.5">
              <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-full border-2 border-emerald-400 overflow-hidden bg-black/40 shadow-lg">
                {room.p1Avatar ? <img src={room.p1Avatar} alt="" className="w-full h-full object-cover" /> : '🤔'}
              </div>
              <strong className="text-emerald-400 text-[10px] sm:text-xs truncate max-w-[85px] sm:max-w-[110px]">{room.p1Name}</strong>
              
              {/* Choice display */}
              {room.status === 'PLAYING' && (
                <div className="text-2xl sm:text-4xl animate-bounce mt-1 sm:mt-2 select-none">
                  {room.p1Choice ? '✔️' : '🤔'}
                </div>
              )}
              {room.status === 'ENDED' && (
                <div className="rps-hand drop text-3xl sm:text-5xl select-none">
                  {getRpsEmoji(room.p1Choice)}
                </div>
              )}
            </div>

            <div className="text-2xl sm:text-4xl font-extrabold italic text-[#ff003c] tracking-wider animate-pulse">VS</div>

            {/* Player 2 Card */}
            <div className="flex flex-col items-center gap-1.5 sm:gap-2.5">
              <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-full border-2 border-[#ff003c] overflow-hidden bg-black/40 shadow-lg">
                {room.p2 ? (
                  room.p2Avatar ? <img src={room.p2Avatar} alt="" className="w-full h-full object-cover" /> : '🤔'
                ) : '❓'}
              </div>
              <strong className={room.p2 ? 'text-[#ff003c] text-[10px] sm:text-xs truncate max-w-[85px] sm:max-w-[110px]' : 'text-white/40 text-[10px] sm:text-xs'}>
                {room.p2 ? room.p2Name : 'Đang chờ...'}
              </strong>
          ...
          <div 
            className="absolute bottom-[24%] sm:bottom-[20%] left-1/2 -translate-x-1/2 text-center text-[10px] sm:text-sm font-bold w-[90%] leading-relaxed"
          ...
          {room.status === 'PLAYING' && !hasMadeSelection && (
            <div className="absolute bottom-3 sm:bottom-5 left-1/2 -translate-x-1/2 flex gap-2 sm:gap-4 z-30 font-mono text-[10px] sm:text-sm">
              <button
                onClick={() => handleMakeChoice('KEO')}
                className="py-1.5 sm:py-2.5 px-3 sm:px-5 bg-zinc-950/40 hover:bg-[#00ff80]/10 border border-[#00ff80] text-white hover:text-[#00ff80] font-black rounded-lg cursor-pointer transition"
              >
                ✌️ KÉO
              </button>
              <button
          ...
          {room.status === 'PLAYING' && hasMadeSelection && !opponentSelected && (
            <div className="absolute bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 text-glow-gold text-[#ffd700] text-[10px] sm:text-xs font-bold uppercase tracking-wider animate-pulse text-center">
          ...
          {/* End Rematch row */}
          {room.status === 'ENDED' && (
            <div className="absolute bottom-2 sm:bottom-4 left-1/2 -translate-x-1/2 flex gap-3 sm:gap-4 z-30 font-mono text-[9px] sm:text-xs">
              <button
                onClick={handleRematch}
                className="py-2 px-4 sm:py-2.5 sm:px-6 bg-yellow-950/30 border border-[#ffd700] text-[#ffd700] text-glow-gold hover:bg-[#ffd700]/10 font-bold rounded-lg uppercase tracking-wider cursor-pointer"
              >
                {room.p1Rematch && isP1 ? 'ĐÃ ĐỒNG Ý' : room.p2Rematch && isP2 ? 'ĐÃ ĐỒNG Ý' : '[ TÁI ĐẤU ]'}
              </button>
              <button
                onClick={handleLeaveRoom}
                className="py-2 px-4 sm:py-2.5 sm:px-6 bg-red-950/30 border border-[#ff003c] text-[#ff003c] hover:bg-[#ff003c] hover:text-white font-bold rounded-lg uppercase tracking-wider cursor-pointer"
              >
                [ RỜI SÒNG ]
              </button>
            </div>
          ...
          {room.status === 'WAITING' && room.creator === uid && room.p2 && (
            <button
              ...
              className="absolute bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 py-2 sm:py-3 px-6 sm:px-8 bg-emerald-950/30 border border-[#00ff80] text-[#00ff80] hover:bg-[#00ff80] hover:text-black font-extrabold rounded-xl text-xs sm:text-base uppercase tracking-widest transition-all cursor-pointer text-glow-green"
            >
              [ KHAI CHIẾN ]
            </button>
          )}
