...
export default function TienLenModal({ uid, user, roomId, onClose, onShowResult }: TienLenModalProps) {
  ...
  return (
    <div className="overlay z-[4500]">
      <div className="glass-box login-panel overflow-y-auto max-h-[90vh] w-[95vw] md:w-full max-w-[1100px] p-6 border-[#32cd32] relative">
        <button 
        ...
        <div className="relative w-full aspect-[16/10] sm:aspect-[16/9] md:h-[480px] max-h-[75vh] min-h-[340px] bg-gradient-to-b from-[#1b5e20] to-[#0d3c15] border-4 sm:border-8 border-[#388e3c] rounded-[30px] sm:rounded-[80px] shadow-[inset_0_0_40px_rgba(0,0,0,0.9)] select-none mb-4 overflow-hidden">
          
          {/* Stake bet banner */}
          <div className="absolute top-2 sm:top-6 left-1/2 -translate-x-1/2 bg-black/60 border border-[#32cd32] py-1 px-4 sm:px-6 rounded-full text-[9px] sm:text-xs font-bold text-[#32cd32] tracking-wider uppercase font-mono z-30 select-none">
            Mức cược: {room.bet.toLocaleString()} PP
          </div>

          {/* Table Cards Played (center spot) */}
          <div className="absolute top-[26%] bottom-[26%] left-[10%] right-[10%] sm:top-[30%] sm:bottom-[30%] sm:left-[15%] sm:right-[15%] border-2 border-dashed border-white/10 rounded-[24px] sm:rounded-[50px] flex items-center justify-center bg-black/10 select-none">
            {room.status === 'ENDED' ? (
              <div 
                className="font-mono text-center text-[9px] sm:text-xs text-white leading-relaxed p-2.5 sm:p-4 bg-black/70 rounded-xl max-h-[100px] sm:max-h-[140px] overflow-y-auto"
              ...
              <div className="flex gap-1.5 sm:gap-2">
                {room.tableCards.map((v, i) => {
                  ...
                  return (
                    <div 
                      key={i} 
                      className={`tl-card w-[45px] sm:w-[65px] h-[68px] sm:h-[95px] border rounded-lg flex flex-col justify-between p-0.5 sm:p-1 bg-white font-mono shadow-md shrink-0 ${
                        isRed ? 'text-red-600' : 'text-black'
                      }`}
                    >
                      <div className="text-xs sm:text-lg font-black text-left pl-0.5 sm:pl-1 leading-none">{r}</div>
                      <div className="text-lg sm:text-2xl text-center leading-none">{s}</div>
                    </div>
                  );
                })}
              </div>
            ) : (
              <div className="text-white/25 italic text-[9px] sm:text-xs tracking-widest uppercase font-mono select-none">
                ĐANG CHỜ ĐỐI THỦ...
              </div>
            )}
          </div>
          ...
          <div className="absolute top-10 sm:top-18 left-3 sm:left-8 flex items-center gap-1.5 sm:gap-2.5 z-20">
            ...
            {room.players && Object.keys(room.players).map((pUid, idx) => {
              ...
              return (
                <div 
                  key={pUid} 
                  className={`p-1.5 sm:p-3 bg-black/85 rounded-lg sm:rounded-xl border border-white/5 flex flex-col items-center w-[85px] sm:w-[110px] text-center ${
                    isTurn ? 'border-yellow-400 shadow-[0_0_10px_rgba(255,215,0,0.5)] animate-pulse' : ''
                  }`}
                >
                  <div className="w-6 h-6 sm:w-9 sm:h-9 rounded-full overflow-y-auto max-h-[90vh] bg-black/30 border border-white/10 mb-0.5 sm:mb-1">
                    {pData.avatar ? <img src={pData.avatar} alt="" className="w-full h-full object-cover" /> : '🤔'}
                  </div>
                  <span className="text-white text-[8px] sm:text-[10px] truncate max-w-[70px] sm:max-w-[95px] font-bold">{pData.name}</span>
                  <span className="text-[7px] sm:text-[9px] text-[#8b949e] mt-0.5 sm:mt-1 uppercase font-black bg-white/5 border border-white/5 py-0.5 px-1 sm:px-1.5 rounded">
                    {passed ? 'BỎ LƯỢT' : pData.hand && pData.hand.length > 0 ? `${pData.hand.length} LÁ` : 'CHỜ CHIA'}
                  </span>
                </div>
              );
            })}
          ...
          <div className="absolute bottom-2 sm:bottom-6 w-full flex flex-col items-center z-30">
            <div className="flex gap-1 sm:gap-1.5 px-3 max-w-full overflow-x-auto h-[95px] sm:h-[130px] items-end pb-1">
              {room.players[uid]?.hand?.map((v, i) => {
                ...
                return (
                  <div
                    ...
                    className={`tl-card cursor-pointer w-[42px] sm:w-[60px] h-[64px] sm:h-[90px] border rounded-lg flex flex-col justify-between p-0.5 sm:p-1 bg-white font-mono shadow-md shrink-0 select-none ${
                      isRed ? 'text-red-600' : 'text-black'
                    } ${isSel ? 'border-yellow-400' : 'border-slate-300'}`}
                  >
                    <div className="text-[10px] sm:text-base font-black text-left pl-0.5 leading-none">{r}</div>
                    <div className="text-base sm:text-2xl text-center leading-none">{s}</div>
                  </div>
                );
              })}
            </div>
