...
export default function FcMobileModal({ uid, user, onClose, onShowResult }: FcMobileModalProps) {
  ...
  return (
    <div className="overlay z-[6000] p-2 sm:p-4 overflow-y-auto">
      <div className="glass-box max-w-[850px] w-full p-3 sm:p-6 border-white/20 relative overflow-y-auto max-h-[92vh] flex flex-col gap-3 sm:gap-4 shadow-3xl animate-scale-up my-auto">
        ...
        <button
          ...
          className={`absolute top-3 right-3 text-[#8b949e] hover:text-white transition duration-200 p-1.5 rounded-lg hover:bg-white/5 cursor-pointer z-20 ${
        ...
        {/* Title Block */}
        <div className="text-center md:text-left select-none pr-8">
          <h2 className="text-white text-glow-blue text-lg sm:text-2xl font-black font-sans uppercase tracking-wider flex items-center justify-center md:justify-start gap-2">
            <Activity className="w-5 h-5 text-emerald-400 animate-pulse shrink-0" />
            FC MOBILE ARENA 2D
          </h2>
          <p className="text-[9px] md:text-xs font-mono text-[#8b949e] uppercase mt-0.5">
            BÓNG ĐÁ SIÊU THỰC // 12 CẦU THỦ CHUYỂN ĐỘNG TỪNG BƯỚC
          </p>
        </div>

        {/* Score Board Display */}
        <div className="grid grid-cols-3 items-center bg-black/60 border border-white/10 rounded-2xl p-2.5 sm:p-4 text-center relative overflow-hidden select-none">
          {/* Blue Column */}
          <div className="flex flex-col items-center">
            <span className="text-glow-blue text-[#00f0ff] font-extrabold text-[10px] sm:text-sm tracking-widest truncate max-w-full">
              XANH S-SYSTEM
            </span>
            <span className="text-[7px] sm:text-[10px] text-slate-400 font-mono mt-0.5">
              Thắng x2
            </span>
          </div>
          ...
          <div className="flex flex-col items-center gap-0.5">
            <div className="flex items-center gap-1.5 sm:gap-4">
              <span className="text-white font-black text-xl sm:text-4xl font-mono tracking-tight">{score.blue}</span>
              <span className="text-[#ffd700] text-glow-gold text-xs sm:text-xl font-mono font-black border border-yellow-500/30 bg-yellow-500/10 py-0.5 px-1.5 sm:px-2.5 rounded-md">
                {matchMin < 90 ? `${matchMin < 10 ? '0' + matchMin : matchMin}:00` : `90+${matchMin - 90}'`}
              </span>
              <span className="text-white font-black text-xl sm:text-4xl font-mono tracking-tight">{score.red}</span>
            </div>
            <span className="text-[8px] font-mono text-emerald-400 font-bold tracking-widest uppercase animate-pulse bg-emerald-950/40 border border-emerald-500/20 px-1.5 py-0.5 rounded-full">
              {phaseText}
            </span>
          </div>
          ...
          {/* Red Column */}
          <div className="flex flex-col items-center">
            <span className="text-glow-red text-[#ff003c] font-extrabold text-[10px] sm:text-sm tracking-widest truncate max-w-full">
              ĐỎ HỌC VIỆN
            </span>
            <span className="text-[7px] sm:text-[10px] text-slate-400 font-mono mt-0.5">
              Thắng x2
        ...
        {/* 2D soccer field */}
        <div
          className={`relative w-full h-[220px] sm:h-[340px] bg-gradient-to-b from-[#1b5e20] to-[#2e7d32] border-2 border-white/60 rounded-2xl overflow-hidden shadow-[inset_0_0_80px_rgba(0,0,0,0.65),_0_20px_40px_rgba(0,0,0,0.8)] select-none shrink-0 ${
