...
export default function BlackjackModal({ uid, user, roomId, onClose, onShowResult }: BlackjackModalProps) {
  ...
  return (
    <div className="overlay z-[4500]">
      <div className="glass-box login-panel overflow-y-auto max-h-[90vh] w-[95vw] md:w-full max-w-[1100px] p-6 border-[#ff8c00] relative">
        <button 
        ...
        <div className="relative w-full aspect-[16/10] sm:aspect-[16/9] md:h-[480px] max-h-[75vh] min-h-[340px] bg-gradient-to-b from-[#0d47a1] to-[#001030] border-4 sm:border-8 border-[#1a237e] rounded-[30px] sm:rounded-[80px] shadow-[inset_0_0_40px_rgba(0,0,0,0.9)] select-none mb-4 overflow-hidden">
          
          {/* Dealer area */}
          <div className="absolute top-2 sm:top-6 left-1/2 -translate-x-1/2 text-center min-h-[90px] sm:min-h-[140px] w-full max-w-[320px]">
            <h3 className="text-white font-bold text-[10px] sm:text-xs uppercase mb-1 sm:mb-2 flex items-center justify-center gap-1.5">
              NHÀ CÁI (BOT) 
              <span className="badge text-[9px] sm:text-[10px] bg-white/10 border border-white text-white py-0.5 px-2 rounded">
                {room.dealer && !room.dealer.isHidden ? getBjInfo(room.dealer.hand).score + ' đ' : '?'}
              </span>
            </h3>

            <div className="flex justify-center gap-1.5 sm:gap-2.5 h-[65px] sm:h-[100px]">
              {room.dealer?.hand ? (
                room.dealer.hand.map((v, i) => {
                  const hide = room.dealer?.isHidden && i === 1;
                  if (hide) {
                    return <div key={i} className="tl-card-back w-[45px] sm:w-[70px] h-[65px] sm:h-[100px] rounded-lg shadow-lg shrink-0" />;
                  }
                  const { r, s, isRed } = getCardRankAndSuit(v);
                  return (
                    <div 
                      key={i} 
                      className={`tl-card shrink-0 w-[45px] sm:w-[70px] h-[65px] sm:h-[100px] border rounded-lg flex flex-col justify-between p-0.5 sm:p-1 bg-white font-mono shadow-md ${
                        isRed ? 'text-red-600' : 'text-black'
                      }`}
                    >
                      <div className="text-xs sm:text-xl font-black text-left pl-0.5 sm:pl-1">{r}</div>
                      <div className="text-lg sm:text-3xl text-center leading-none">{s}</div>
                    </div>
                  );
                })
              ) : (
                <div className="text-white/20 italic text-[9px] sm:text-xs flex items-center justify-center border border-dashed border-white/10 rounded-lg w-[100px] sm:w-[140px] h-[65px] sm:h-[100px]">
                  Chờ chia bài...
                </div>
              )}
            </div>
          ...
          {room.status === 'ENDED' ? (
            <div 
              className="absolute top-[38%] sm:top-[42%] left-1/2 -translate-x-1/2 -translate-y-1/2 font-mono text-center text-[9px] sm:text-xs text-white max-w-[80%] leading-relaxed max-h-[80px] sm:max-h-[140px] overflow-y-auto bg-black/70 p-2.5 sm:p-4 border border-yellow-500/20 rounded-xl"
            ...
            <div className="absolute top-[38%] sm:top-[42%] left-1/2 -translate-x-1/2 -translate-y-1/2 font-mono text-center text-xs sm:text-lg text-glow-gold text-[#ffd700] font-black animate-pulse">
              NHÀ CÁI ĐANG RÚT BÀI...
            </div>
          ) : (
            <div className="absolute top-[38%] sm:top-[42%] left-1/2 -translate-x-1/2 -translate-y-1/2 font-mono text-center text-[9px] sm:text-xs text-white/30 tracking-widest uppercase">
              NHÀ CÁI BAO SÂN - S-SYSTEM 88
            </div>
          )}

          {/* Player areas */}
          <div className="absolute bottom-2 sm:bottom-6 w-full flex justify-around items-end px-3 sm:px-6 min-h-[130px] sm:min-h-[220px]">
            {room.players && Object.keys(room.players).map((pUid) => {
              ...
              return (
                <div key={pUid} className="flex flex-col items-center">
                  ...
                  <div className="flex justify-center gap-1 min-h-[55px] sm:min-h-[90px] mb-1 sm:mb-3">
                    {pData.hand?.map((v, i) => {
                      const { r, s, isRed } = getCardRankAndSuit(v);
                      return (
                        <div 
                          key={i} 
                          className={`tl-card w-[36px] sm:w-[55px] h-[52px] sm:h-[80px] border rounded-md flex flex-col justify-between p-0.5 sm:p-1 bg-white font-mono shadow-sm shrink-0 ${
                            isRed ? 'text-red-600' : 'text-black'
                          }`}
                        >
                          <div className="text-[10px] sm:text-sm font-black text-left pl-0.5 leading-none">{r}</div>
                          <div className="text-sm sm:text-xl text-center leading-none">{s}</div>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  {/* Player details */}
                  <div className={`p-1.5 sm:p-3 bg-black/85 rounded-lg sm:rounded-xl border border-white/5 flex flex-col items-center w-[90px] sm:w-[120px] transition-all relative ${
                    isTurn ? 'border-yellow-400 shadow-[0_0_15px_rgba(255,215,0,0.3)] animate-pulse scale-105' : ''
                  }`}>
                    <div className="w-6 h-6 sm:w-10 sm:h-10 rounded-full border border-white/20 overflow-y-auto max-h-[90vh] mb-0.5 sm:mb-1.5 shadow">
                      {pData.avatar ? <img src={pData.avatar} alt="" className="w-full h-full object-cover" /> : '🤔'}
                    </div>
                    <b className="text-white text-[8px] sm:text-[11px] truncate max-w-[75px] sm:max-w-[100px]" title={pData.name}>{pData.name}</b>
                    <span className="text-[8px] sm:text-[10px] text-[#ffd700] font-black mt-0.5">{room.bet.toLocaleString()} PP</span>
                    
                    {/* Status badge */}
                    <span className="mt-1 sm:mt-1.5 py-0.5 px-1 sm:px-2 border border-white/10 text-[7px] sm:text-[9px] font-black uppercase text-[#8b949e] rounded bg-white/5">
                      {statusLabel}
                    </span>
                  </div>
                </div>
          ...
          {isMyTurn && room.status === 'PLAYING' && (
            <div className="absolute top-[48%] sm:top-[50%] left-1/2 -translate-x-1/2 flex gap-3 sm:gap-4 z-40 font-mono text-xs sm:text-sm">
              <button
                onClick={handleHit}
                disabled={isProcessingAction}
                className="py-1.5 sm:py-3 px-4 sm:px-8 bg-emerald-950/40 border sm:border-2 border-dashed border-[#00ff80] text-[#00ff80] text-glow-green hover:bg-[#00ff80]/15 font-black uppercase rounded-lg sm:rounded-xl tracking-wider cursor-pointer"
              >
                [ 👆 RÚT BÀI ]
              </button>
              <button
                onClick={handleStand}
                disabled={isProcessingAction}
                className="py-1.5 sm:py-3 px-4 sm:px-8 bg-red-950/40 border border-[#ff003c] text-[#ff003c] text-glow-red hover:bg-[#ff003c]/15 font-black uppercase rounded-lg sm:rounded-xl tracking-wider cursor-pointer"
              >
                [ ✋ DẰN BÀI ]
              </button>
            </div>
          ...
          {room.status === 'WAITING' && room.creator === uid && (
            <button
              ...
              className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 py-2.5 sm:py-3.5 px-6 sm:px-8 bg-yellow-950/30 border border-[#ff8c00] text-[#ff8c00] hover:bg-[#ff8c00] hover:text-black text-xs sm:text-base font-black rounded-lg sm:rounded-xl uppercase tracking-widest transition-all cursor-pointer text-glow-gold"
            >
              [ BẮT ĐẦU VÁN BÀI ]
            </button>
          )}
