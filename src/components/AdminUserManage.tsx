...
export default function AdminUserManage({ onShowResult }: AdminUserManageProps) {
  ...
  const [resetRequests, setResetRequests] = useState<PasswordResetReq[]>([]);
  ...
  useEffect(() => {
    ...
    const loadUsers = async () => {
      ...
      try {
        ...
        const reqSnap = await get(ref(db, 'password_reset_requests'));
        if (reqSnap.exists()) {
          const reqList: PasswordResetReq[] = [];
          reqSnap.forEach((child) => {
            const val = child.val();
            if (val.status === 'PENDING') {
              reqList.push({ id: child.key!, ...val });
            }
          });
  ...
  const handleApprovePasswordReset = async (req: PasswordResetReq) => {
    try {
      // Reset user password to '123'
      await update(ref(db, `users/${req.uid}`), { pass: '123' });
      // Update request status
      await update(ref(db, `password_reset_requests/${req.uid}`), { status: 'APPROVED' });

      setResetRequests(prev => prev.filter(r => r.uid !== req.uid));
      setStudents(prev => prev.map(s => s.id === req.uid ? { ...s, pass: '123' } : s));

      if (onShowResult) {
        onShowResult('DUYỆT ĐỔI MẬT KHẨU 🎉', `Đã đổi mật khẩu cho UID ${req.uid} (${req.name}) về "123"!`, true);
      } else {
        alert(`Đã đổi mật khẩu cho UID ${req.uid} về "123"!`);
      }
    } catch (err) {
      if (onShowResult) {
        onShowResult('THẤT BẠI ❌', 'Lỗi duyệt đổi mật khẩu!', false);
      }
    }
  };
  ...
  return (
    <div id="admin-user-manage" className="glass-box p-6 border-white/10 bg-black/40 font-mono text-xs text-slate-200">
      ...
      {/* PENDING PASSWORD RESET REQUESTS (QTV APPROVAL) */}
      {resetRequests.length > 0 && (
        <div className="mb-6 p-4 bg-gradient-to-r from-yellow-950/40 via-amber-950/20 to-black border-2 border-yellow-500/50 rounded-xl">
          <div className="flex items-center gap-2 mb-3">
            <Key className="w-4 h-4 text-yellow-400 animate-pulse" />
            <span className="text-yellow-400 font-black uppercase text-xs tracking-wider">
              YÊU CẦU ĐỔI MẬT KHẨU TỪ SINH VIÊN ({resetRequests.length})
            </span>
          </div>

          <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
            {resetRequests.map((req) => (
              <div key={req.uid} className="flex items-center justify-between p-2.5 bg-black/70 border border-yellow-500/30 rounded-lg text-xs">
                <div>
                  <span className="text-white font-bold">{req.name}</span>
                  <span className="text-yellow-400 font-mono ml-2">UID: {req.uid}</span>
                  <span className="text-slate-400 text-[10px] ml-2 block sm:inline">
                    Gửi lúc: {new Date(req.requestedAt).toLocaleTimeString('vi-VN')}
                  </span>
                </div>
                <button
                  onClick={() => handleApprovePasswordReset(req)}
                  className="py-1.5 px-3 bg-gradient-to-r from-yellow-500 to-amber-500 hover:from-yellow-400 hover:to-amber-400 text-black font-black text-[10px] uppercase rounded-lg cursor-pointer shadow-md transition-all active:scale-95 whitespace-nowrap"
                >
                  ⚡ DUYỆT RESET VỀ 123
                </button>
              </div>
